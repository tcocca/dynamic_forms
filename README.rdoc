...
* Be able to re-order the form fields and form field options
...
* If the Radio Button field is required make the first option by position (order) set as checked by default
  * if the user selects another option and the form did not pass validation make sure that chosen value is selected (obviously)

...
* Required (Text Field, Text Area, File Field, Select, Check Box Group)
...
* More Field Choices
  * Radio Button Field (This would be a group)
...
* Add the ability to choose if you want an email to be sent when submit fills out the form
  * Specify the email address on a per-form basis
  * Attach results of file_fields as attachments in the email (Maybe?)

* Add the ability to specify "Thank You" text to display on the template when a form has been submitted

* Add a configuration initializer
  * Set the File Types, Validations and Default Mime Types in the configuration object instead of class constants
